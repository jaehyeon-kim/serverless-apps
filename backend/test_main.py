...
def test_create_item(todo_item):
    ...
    result = resp.json()
    pytest.created_at = result["item"]["created_at"]
    assert resp.status_code == 200
...
def test_update_item(todo_item):
    ...
    ) > datetime.strptime(result["item"]["created_at"], TodoItems().dt_format)
...
def test_read_item(todo_item, all_items):
    resp = get_items_resp(
        username=todo_item["username"], created_at=pytest.created_at, all_items=all_items
    )
    assert resp.status_code == 200
